...
def clean_text(text):
    # 페이지 번호, 법제처, 국가법령정보센터, 민사소송법 등 불필요한 텍스트 제거
    text = re.sub(r'법제처\s+\d+\s+국가법령정보센터', '', text)
    text = re.sub(r'민사소송법\s*', '', text)
...
def pdf_to_json(pdf_path):
    ...
    with open(pdf_path, 'rb') as file:
        ...
        # 정제된 텍스트를 딕셔너리에 저장
        text_data = {"civil_procedure_law": full_text.strip()}
...
# PDF 파일 경로
pdf_path = '/home/user/hjkim/crawling/pdf/민사소송법.pdf'
